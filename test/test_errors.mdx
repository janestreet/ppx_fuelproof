...
Error: This variant or record definition does not match that of type
       ...
       Constructors do not match:
         A : ('a : value mod portable). 'a list/2 -> t
       is not the same as:
         A : ('a : value mod portable). 'a list/1 -> t
...
Errors on using `non_float` with an unboxed record containing a float:

```ocaml
type%fuelproof t : value mod non_float = { x : float } [@@unboxed]
```
```mdx-error
...
Error: Bad layout annotation:
         The layout of float is value
           because it is the primitive type float.
         But the layout of float must be a sublayout of any non_float
           because of the annotation on the wildcard _ at line 1, characters 48-53.
```

Errors on using `non_float` with an unboxed variant containing a float:

```ocaml
type%fuelproof t : value mod non_float = T of float [@@unboxed]
```
```mdx-error
...
Error: Bad layout annotation:
         The layout of float is value
           because it is the primitive type float.
         But the layout of float must be a sublayout of any non_float
           because of the annotation on the wildcard _ at line 1, characters 47-52.
```

Errors on using `non_float` with an unboxed variant with inline record containing a float:

```ocaml
type%fuelproof t : value mod non_float = T of { x : float } [@@unboxed]
```
```mdx-error
...
Error: Bad layout annotation:
         The layout of float is value
           because it is the primitive type float.
         But the layout of float must be a sublayout of any non_float
           because of the annotation on the wildcard _ at line 1, characters 53-58.
```

Errors on using `non_float` with an unboxed GADT with existential that doesn't have
`non_float` annotation:

```ocaml
type _ u
type%fuelproof t : value mod non_float = T : _ u -> t [@@unboxed]
```
```mdx-error
Line 2, characters 48-51:
Error: Bad layout annotation:
         The layout of 'a u is value
           because of the definition of u at line 1, characters 1-9.
         But the layout of 'a u must be a sublayout of any non_float
           because of the annotation on the wildcard _ at line 2, characters 48-51.
```
